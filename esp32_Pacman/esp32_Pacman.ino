...
/******************************************************************************/
/*                 ESP32-S2 PAC-MAN     REMASTER BY #DROPES                   */
/******************************************************************************/
...
/******************************************************************************/  
    ...
    const uint32_t Sample_Size_beginning = sizeof(_beginning) - 1;
    const uint32_t Sample_Size_eatdot = sizeof(_eatdot) - 1;
    const uint32_t Sample_Size_death = sizeof(_death) - 1;
    const uint32_t Sample_Size_eatfruit = sizeof(_eatfruit) - 1;
    const uint32_t Sample_Size_eatghost = sizeof(_eatghost) - 1;
    const uint32_t Sample_Size_extrapac = sizeof(_extrapac) - 1;
    const uint32_t Sample_Size_intermission = sizeof(_intermission) - 1;
    const uint32_t Sample_Size_powerup = sizeof(_powerup) - 1;
    const uint32_t Sample_Size_gethi = sizeof(_gethi) - 1;

    const uint32_t Sample_Size_siren = sizeof(_siren) - 1;        // Sound_loop 1
    const uint32_t Sample_Size_eyeback = sizeof(_eyeback) - 1;    // Sound_loop 2   
                                                                  // Sound_loop 0 = no loop  

    uint32_t Sample_Pos_beginning = Sample_Size_beginning;        //beginning : start new game
    uint32_t Sample_Pos_eatdot = Sample_Size_eatdot;              //eatdot : eat dot
    uint32_t Sample_Pos_death = Sample_Size_death;                //death : pacman die
    uint32_t Sample_Pos_eatfruit = Sample_Size_eatfruit;          //eatfruit : eat fruit
    uint32_t Sample_Pos_eatghost = Sample_Size_eatghost;          //eatghost : eat ghost
    uint32_t Sample_Pos_extrapac = Sample_Size_extrapac;          //extrapac : additional life at 10.000pts
    uint32_t Sample_Pos_intermission = Sample_Size_intermission;  //intermission : new level
    uint32_t Sample_Pos_gethi = Sample_Size_gethi;                //pacman : pac-man enter in hiscore
    uint32_t Sample_Pos_powerup = Sample_Size_powerup;            //powerup : eat big dot
    uint32_t Sample_Pos_siren = Sample_Size_siren;                //normal enviroment
    uint32_t Sample_Pos_eyeback =  Sample_Size_eyeback;           //eyeback : eyes ghost returning to pit

    hw_timer_t *Timer0_Cfg = NULL;
    uint8_t Sound_loop = 0;

    #define SOUND_ON 6
/******************************************************************************/
/*   Battery monitor                                                          */
/******************************************************************************/ 
#define Pin_bat 7 //12bits ADC
...
u_short _frightenedghostTimer = 0;
/******************************************************************************/
/*   GAME - Sprite Class                                                      */
/******************************************************************************/
      ...
      if (who != PACMAN)
      {
        ...
        switch (state)
        {
          case FrightenedState:
            //60FPS x 10 : frightenedghostTimer = 600
            if (_frightenedghostTimer > 100) {
              bits = FRIGHTENEDGHOSTSPRITE + (p & 1); // frightened
              palette2 = FRIGHTENEDPALETTE;}
            else if (_frightenedghostTimer % 5 != 0) {
              bits = FRIGHTENEDGHOSTSPRITE + (p & 1); // frightened
              palette2 = FRIGHTENEDPALETTE;
            }
            break;
...
//      }
    ...
    void DrawBG(byte cx, byte cy, byte* tile)
    {
      ...
      if (cy == 20 && cx >= 11 && cx < 17)
      {
        ...
        else if (DEMO == 1 && cx == 11) b = 0; // hiscore
...
//--------------------------------------------------------------------------------------------
    ...
    void UpdateTimers()
    {
      ...
      _frightenedghostTimer = _frightenedTimer;
    }
//--------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------
    ...
    void PacmanAI()
    {
      ...
      else
      {
        ...
        if (dist == 0x7FFF) {
          GAMEWIN = 1; // No dots, GAME WIN!  sometimes!!!!!!!!!!!!!!!!
        }
...
//--------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------
    void Scatter(Sprite* s)   // dispersão
    {
      ...
      for (byte i = 0; i < 4; i++)
      {
        ...
        if (s->state == DeadEyesState)
        {
          if (s->cx == 14 && s->cy == 17) // returned to pen
          {
            ...
            s->pentimer = 200;
            Sound_loop = 1;               //siren sound
          }
          else
          {
            s->Target(14, 17);            // target pen
            Sound_loop = 2;               //eyeback sound
          }
          continue;           //
        }
...
//--------------------------------------------------------------------------------------------
    void PackmanDied() {  // Noooo... PACMAN DIED :(
      ...
      if (LIFES <= 0) {
       ...
       if (_hiscore_new > _hiscore) { // Save hiscore
          _hiscore = _hiscore_new;
          EEPROM.writeLong(0,  _hiscore);
          EEPROM.commit();
          Sound_loop = 0;
         
          if (digitalRead(SOUND_ON) == 1) {
            Sample_Pos_gethi = 0;              
            while (Sample_Pos_gethi < Sample_Size_gethi) delay(100);
          }
       }
        ...
        if (s->_x + SPEED >= pacman->_x && s->_x - SPEED <= pacman->_x && s->_y + SPEED >= pacman->_y && s->_y - SPEED <= pacman->_y)
        {
          if (s->state == FrightenedState)
          {
            ...
            if (s->state == DeadNumberState || s->state == FrightenedState || s->state == DeadEyesState) {
              ...
              Sound_loop = 0;
              Sample_Pos_death = 0;              
              while (Sample_Pos_death < Sample_Size_death) delay(100);
              PackmanDied();
              Sound_loop = 1;
            }
          }
...
//--------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------
    void SetBatteryChar(byte i, signed char c)
    {
      if (_batteryStr[i] == c)
        return;
      _batteryStr[i] = c;
      Mark(i);  //Battery status
    }
//--------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------
    void Score(int16_t delta)
    {
      ...
      if (DEMO == 0) {
        _score += delta;
        _hiscore_new = _score; // not anymore :)
      }

      if (_score > _lifescore && _score % 10000 > 0) {
        _lifescore = (_score / 10000 + 1) * 10000;

        LIFES++; // EVERY 10000 points = 1UP (Just on 1st 10000 rule ?!)
        Sample_Pos_extrapac = 0;
   ...
   // Check battery
      ...
      // 3.0V = analog 1422
      //1990 - 1422 = 568
      //100% / 568 = 0,176056338028169
      //(analog - 1422) * 0.176 = %

    _battery = (analogReadMilliVolts(Pin_bat) - 1422) * 0.176;

    if (_battery > 100) _battery = 100;
    if (_battery < 0) _battery = 0;

    sprintf(str, "%ld", _battery);
    
    for (i = 0; i<4 ; i++)
      SetBatteryChar(i, str[i]);
    }
...
//--------------------------------------------------------------------------------------------
    void Init()
    {
      ...
      if ((DEMO == 0) && (LEVEL == 1) && (digitalRead(SOUND_ON) == 1)) {
          Sound_loop = 0; //no sound loop

          Sample_Pos_siren = Sample_Size_siren; // Remove DEMO posisitons
          Sample_Pos_eyeback = Sample_Size_eyeback;

          Sample_Pos_beginning = 0;
          while (Sample_Pos_beginning < Sample_Size_beginning) delay(100); // without delay, brakes
          Sound_loop = 1; //siren
      }
...
//--------------------------------------------------------------------------------------------
    void Step()
    {
      ...
      if (GAMEWIN == 1) {
        ...
        if (digitalRead(SOUND_ON) == 1) {
          Sound_loop = 0;
          Sample_Pos_intermission = 0;            
          while (Sample_Pos_intermission < Sample_Size_intermission) delay(100);
          Sound_loop = 1; 
        }       
        Init();
      }
      ...
      // Pause
      if (but_B && DEMO == 0 && GAMEPAUSED == 0) {
        but_B = false;
        GAMEPAUSED = 1;
        Sound_loop = 0;
      } else if (but_B && DEMO == 0 && GAMEPAUSED == 1) {
        but_B = false;
        GAMEPAUSED = 0;
        Sound_loop = 1;
...
{
  uint8_t Result1; //environment
  uint8_t Result2; //effects

  int16_t Soma2;
  
  if (Sample_Pos_beginning < Sample_Size_beginning) Result1 = (uint8_t) *(_beginning + Sample_Pos_beginning++);
  if (Sample_Pos_intermission < Sample_Size_intermission) Result1 = (uint8_t) *(_intermission + Sample_Pos_intermission++);

  if (Sample_Pos_eatdot < Sample_Size_eatdot) Soma2 += ((uint8_t) *(_eatdot + Sample_Pos_eatdot++)) - 0x80;
  if (Sample_Pos_death < Sample_Size_death) Soma2 += ((uint8_t) *(_death + Sample_Pos_death++)) - 0x80;
  if (Sample_Pos_eatfruit < Sample_Size_eatfruit) Soma2 += ((uint8_t) *(_eatfruit + Sample_Pos_eatfruit++)) - 0x80;
  if (Sample_Pos_eatghost < Sample_Size_eatghost) Soma2 += ((uint8_t) *(_eatghost + Sample_Pos_eatghost++)) - 0x80;
  if (Sample_Pos_extrapac < Sample_Size_extrapac) Soma2 += ((uint8_t) *(_extrapac + Sample_Pos_extrapac++)) - 0x80;
  if (Sample_Pos_powerup < Sample_Size_powerup) Soma2 += ((uint8_t) *(_powerup + Sample_Pos_powerup++)) - 0x80;

  if (Sample_Pos_siren < Sample_Size_siren) Result1 = (uint8_t) *(_siren + Sample_Pos_siren++);
  if (Sample_Pos_eyeback < Sample_Size_eyeback) Result1 = (uint8_t) *(_eyeback + Sample_Pos_eyeback++);

  if (Sample_Pos_gethi < Sample_Size_gethi) Result1 = (uint8_t) *(_gethi + Sample_Pos_gethi++); 

  // LOOP MODE
  switch(Sound_loop) {
  case 1:
    if (Sample_Pos_siren == Sample_Size_siren) Sample_Pos_siren = 0; 
    break;
  case 2:
    if (Sample_Pos_eyeback == Sample_Size_eyeback) Sample_Pos_eyeback = 0;
    break;
  }

  Result2 = Soma2 + 0x80;

  if (DEMO == 0) { //no sounds on demo
    dac_output_voltage(DAC_CHANNEL_1, Result1); 
    dac_output_voltage(DAC_CHANNEL_2, Result2); 
  } 
}
...
void setup() {
  ...
  tft.setRotation(2); //0=pins down , 2=pins up
  ...
  //sound int
  Timer0_Cfg = timerBegin(0, 64, true);
  timerAttachInterrupt(Timer0_Cfg, &Timer0_ISR, true);
  timerAlarmWrite(Timer0_Cfg, 160, true);
  timerAlarmEnable(Timer0_Cfg);
  dac_output_enable(DAC_CHANNEL_1);
  dac_output_enable(DAC_CHANNEL_2);

  pinMode(SOUND_ON, INPUT_PULLDOWN);
}
...
void KeyPadLoop(){
  if (digitalRead(PIN_A) == 0) { ClearKeys(); but_A=true; delay(300); } //else but_A=false;
  if (digitalRead(PIN_B) == 0) { ClearKeys(); but_B=true; delay(300); } //else but_B=false;
  if (digitalRead(PIN_UP) == 0) { ClearKeys(); but_UP=true; }           //else but_UP=false;
  if (digitalRead(PIN_DOWN) == 0) { ClearKeys(); but_DOWN=true; }       //else but_DOWN=false;
  if (digitalRead(PIN_LEFT) == 0) { ClearKeys(); but_LEFT=true; }       //else but_LEFT=false;    
  if (digitalRead(PIN_RIGHT) == 0) { ClearKeys(); but_RIGHT=true; }     //else but_RIGHT=false; 
}
