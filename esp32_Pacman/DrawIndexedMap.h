...
void drawIndexedmap(Adafruit_ST7789* tft, uint8_t* indexmap, int16_t x, uint16_t y) {
  byte i = 0;
  ...
  for (byte tmpY = 0; tmpY < 8; tmpY++) {
    ...
    for (byte tmpX = 0; tmpX < 8; tmpX++) {
      ...
      if ((color != next_color && width >= 1) || tmpX == 7) {
        //tft->drawFastHLine(x + tmpX - width + 1, y + tmpY, width - 1, color); ILI9328
        tft->drawFastHLine(x + tmpX - width , y + tmpY, width , color);
        color = next_color;
        width = 0;
      }
      width++;
